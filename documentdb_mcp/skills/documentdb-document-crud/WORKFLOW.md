...
```
documentdb_crud action=insert_one  database_name=app collection_name=users document={"email":"[REDACTED_EMAIL]","active":true}
documentdb_crud action=find_one    database_name=app collection_name=users filter={"email":"[REDACTED_EMAIL]"}
```
Paged, sorted query:
```
