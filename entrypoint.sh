...
# 3. Default to documentdb-mcp (set in Dockerfile CMD, so $1 is usually this if no args)

COMMAND="${CMD:-$1}"

# If we are using the default from Dockerfile ($1) or an explicit argument, we shift it off
# If we are using ENV CMD, we still expect $1 to be the default "documentdb-mcp" or empty, so shifting is generally safe if we assume standard usage.
...
# So if we use -e CMD=a2a, and run `docker run ...`, $1 IS "documentdb-mcp". So we MUST shift.
...
# Check if $1 is "documentdb-mcp" (the default). If so, shift it.
# If $1 is NOT "documentdb-mcp" (e.g. it's "--flag" or another command):
...
if [ "$COMMAND" = "documentdb-mcp" ]; then
    exec documentdb-mcp \
    --transport "${TRANSPORT}" \
    --host "${HOST}" \
    --port "${PORT}" \
...
elif [ "$COMMAND" = "documentdb-a2a" ]; then
